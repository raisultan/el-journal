...
class JournalContainer extends Component {
  addCellContentToStudentsData = (journal, sorterFunc) => {
    ...
    studentsData = addAbsentDatesToMarksOfStudentObjects(
      studentsData,
      weekList
    );
  ...
  render() {
    ...
    if (
      journal &&
      journal.data &&
      Object.keys(journal.data).length !== 0 &&
      journal.data.constructor !== Object
    ) {
    ...
    const renderingComponent = () => {
      let comp = <EditableTable data={data} columns={columns} />;

      if (!displayJournal) {
        comp = (
          <StyledCentererWrapper>
            <h2>
              Для вывода журнала выберите соответствующие предмет и класс!
            </h2>
          </StyledCentererWrapper>
        );
      } else if (pending) {
